...
As background, `GLP-1 agonists` are a class of medications that are clinically used to manage blood sugar (glucose) levels in people with Type 2 diabetes and can also help treat obesity. The most commonly known GLP-1 agonist is `Ozempic`. The `inclusion criteria` for this cohort was having a diagnosis of 2 diabetetes, actively using `insulin`, and being aged 65+ at baseline. Meanwhile, the `exclusion criteria` is people who already have a heart disease diagnosis or are enrolled in a heart disease prevention program at hospital system.
