...
The function is will have 2 inputs.

\- Inputs: (i) 1 `List` of vectors (ii) 1 `Vector` of characters
...
hospitals_ca <- read_csv(here(week, "data", "Hospitals.csv")) %>%
  filter(STATE== 'CA')
```
...
# Define the vector of data frames names
...
```{r}
knitr::include_graphics(here(week, "images", "automation_function_1.png"))
```
...
```{r}
knitr::include_graphics(here(week, "images", "automation_function_2.png"))
```

Inputs: (i) `List` containing each of the `Data Frames` (ii) `Vector` of SPDF names

Transformation: Converting data frames into SPDFs

Output: (i) `List` containing each of the `Spatial Data Frames` with assigned SPDF names

```{r}
createSPDF <- function(list_of_dfs, spdf_names) {
  ...
  for (i in seq_along(list_of_dfs)) {
    ...
    spdf <- sp::SpatialPointsDataFrame(coords = coord_cols, 
                                       data = info_cols, 
                                       proj4string = p4s)
...
list_of_spdfs <- createSPDF(list_of_dfs, vector_spdf_names)
```
...
## Create Function to Add Markers Using Preprocessed Data
...
## Loop through Data with Function
