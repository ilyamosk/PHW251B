...
load(file = here(week, "data", "ca_aqi.rdata"))
...
library(dplyr)      # for data manipulation with %>% and mutate()
library(ggplot2)    # for data visualization (grammar of graphics)
library(forcats)    # for factor reordering (fct_rev)
library(stringr)    # for string wrapping of long labels
library(plotly)     # for interactive visualizations (ggplotly)

# Reverse the order of categories so bars display from top to bottom
aqi_plot_data <- aqi_categories_percentage %>%
  mutate(
    # If 'category' is already a factor, just reverse it
    # Otherwise, first convert to factor and then reverse
    category = if (is.factor(category)) forcats::fct_rev(category)
               else forcats::fct_rev(factor(category))
  )

# Build the static ggplot object
bar_static <- ggplot(
  data = aqi_plot_data,          # dataset for plotting
  aes(
    y = category,                # categories on the y-axis
    x = percentage,              # percentage values on the x-axis
    fill = category,             # fill color by category
    text = paste("Percentage:", percentage) # custom hover text for plotly
  )
) +
  # Draw bars with semi-transparent fill and colored outlines
  geom_col(aes(color = category), linewidth = 1, alpha = 0.5) +
  
  # Add white percentage labels inside bars, nudged slightly right
  geom_label(
    aes(label = paste0(percentage, "%")),  # label text: "##%"
    color = "white",                       # label text color
    hjust = -0.25,                         # horizontal adjust (move right)
    fontface = "bold"                      # bold font for emphasis
  ) +
  
  # Wrap long y-axis labels at ~10 characters to prevent overlap
  scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 10)) +
  
  # Manual color scales (fill + outline) to match AQI categories
  scale_fill_manual(values = c("green3", "gold", "orange", "red", "purple", "maroon")) +
  scale_color_manual(values = c("green3", "gold", "orange", "red", "purple", "maroon")) +
  
  # Adjust x-axis: start at 0, end at 65, with no extra padding left
  scale_x_continuous(
    limits = c(0, 65),                          # axis range
    expand = expansion(mult = c(0, 0), add = c(0, 1)) # remove left padding, add small right padding
  ) +
  ...
  theme(
    ...
    axis.line.x = element_line(color = "grey90", linewidth = 1.25), # subtle x-axis line
    axis.title = element_text(color = "grey60", face = "bold"),     # bold grey axis titles
    axis.text  = element_text(color = "grey60", face = "bold")      # bold grey axis text
  ) +
  
  # Remove redundant legends (fill and color already obvious)
  guides(fill = "none", color = "none") +
  
  # Label axes (y left blank since categories are self-explanatory)
  labs(y = NULL, x = "Percentage (%)")

# Convert the static ggplot to an interactive plotly object
p <- ggplotly(bar_static, tooltip = "text") %>%  # show custom hover text
  layout(showlegend = FALSE)                     # explicitly hide legend
