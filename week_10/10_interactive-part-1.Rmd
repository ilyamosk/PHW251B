...
# Create list of Bay Area cities plus LA that we'll filter for
bay_area_la_cities <- c("San Francisco", "San Jose", "Santa Rosa", "Vallejo", 
                        "Napa", "Santa Cruz", "Los Angeles")

# Filter for just the cities in bay area and LA
ca_aqi <- ca_aqi %>%
  filter(city_ascii %in% bay_area_la_cities)
...
# Create list of Bay Area Counties plus LA Countythat we'll filter for
bayarea_counties_plus_la <- c("San Francisco", "Alameda", "Contra Costa", "Santa Clara", "San Mateo", 
                              "Marin", "Solano", "Sonoma", "Napa", "Santa Cruz", "San Benito", "Los Angeles")
...
counties <- tigris::counties(state = "California", progress_bar = FALSE) %>%
  ...
  filter(name %in% bayarea_counties_plus_la) %>% 
  rename(county = name) %>% 
...
# Filter for just the cities in bay area and LA
ca_aqi_filtered <- ca_aqi %>%
  filter(city_ascii %in% bay_area_la_cities)

# Convert the ca_aqi dataset into an sf object, setting crs = 4269 (same as counties)
ca_aqi_geo <- sf::st_as_sf(ca_aqi_filtered, coords = c("lng", "lat"), crs = 4269)
