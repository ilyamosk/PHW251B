...
Let's try combine some of these aesthetics from `Module: Scatterplots`! Let's create a new plot from the `cities_clean` dataset displaying the relationship between `hours_sun` (x-axis) and `obesity` (y-axis). Let's also set the alpha to be responsive to `gym_cost` to display a third variable.
...
Now plot the relationship between `hours_sun` (x-axis) and `avg_hours_worked` (y-axis). Take a look at the documentation for `geom_smooth` and find and implement the argument for the LOESS method. Then, create a **horizontal** reference line at the 90th percentile of `avg_hours_worked`.  (Hint - use `?geom_smooth`)
