...
You are going to access `DataHub` at via the bCourses site in the left side panel by clicking `DataHub` and then clicking "Open in New tab". 

# Saving a copy of your Rmd files for `exercises` and `assignments`
...
**NOTE** - As with the `here` package, we have also introduced the `janitor` package into the `R Markdown` files that you are going to use for this course. Essentially, we replaced this complicated `rename_with(~ tolower(gsub(" ","_", .x, fixed=TRUE)))` code with `clean_names` throughout the course because it is an industry standard package that we wanted you to learn and to simplify the code where we thought was appropriate :)
...
`knitr` is the package that renders/knits your code chunks, text, and outputs together into a  document (in this course we render to HTML). 

## Global vs Local Chunk Settings

### Global Setting

`knitr` applies **Global Settings** that you define with `knitr::opts_chunk$set()`, which affect all chunks by default.

Here are the **Global Settings** for this file.
...
However, you can always override these with **Local Settings** inside an individual chunk. For example, even if your **Global Settings** include `echo = TRUE`, you can set `echo = FALSE` in a specific chunk to hide its code. One important detail is that the very first code chunk is not affected by **Global Settings** so you’ll often see **Local Settings** applied there directly as seen above.
...
- `tidy.opts`-> allows you to set right margin for rendering via
  - `list(width.cutoff = #))`
