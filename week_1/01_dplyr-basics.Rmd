...
**A NOTE FROM THE INSTRUCTORS** - Parts of this `RMarkdown` file have been updated from the accompanying video to elevate your learning experience as mentioned in `Module 0: Orientation`. Essentially, we replaced BaseR functions with the available equivalent functions from the `tidyverse` because it is best practice in industry. Some reasons for these best practices include better support for `tidyverse` workflows with the `%>%` operator, is incredibly well documented, and runs faster when working with big data. Therefore, the code that you may see in the recording may be slightly different than the code in the `R Markdown` file, however it accomplishes the same thing with better methodologies :)
...
asthma <- read_csv(here(week, "data", "asthma.csv")) %>% clean_names()
