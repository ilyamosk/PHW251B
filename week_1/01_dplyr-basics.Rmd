...
This R Markdown (or .rmd) is a brief review of R programming basics using the dplyr (and greater tidyverse collection) library for data wrangling and cleaning, and is the first of a series of review modules for PHW251B: Data Visualization for Public Health. Students are assumed to have completed some coursework in R, including but not limited to PH142 and/or PHW251. If any wording or concepts are less familiar, please feel free to review said courses. 

Please reach out to Dr. Rain Mocello or the course GSI for any questions you may have, or for assistance in accessing materials for review or practice.
...
- Conditional mutates with `ifelse` and `case_when`
...
We will use the `read_csv` function from `tidyverse's` `readr` package to read in the `asthma_mortality.csv` dataset. 

[ **NOTE** - Parts of this RMarkdown file have been updated from the accompanying video to elevate your learning experience. Essentially, we replaced BaseR functions with the available equivalent functions from the `tidyverse` because it is best practice in industry. Some reasons for these best practices include better support for `tidyverse` workflows with the `%>%` operator, is incredibly well documented, and runs faster when working with big data :) ]

`clean_names` from the `janitor` package standardizes data-frame column names so they’re safe, consistent, and easy to work with. It lowercases names, converts spaces/hyphens/punctuation to underscores, removes/normalizes special characters (e.g., accents), collapses multiple separators, trims leading/trailing underscores, and guarantees uniqueness by appending numeric suffixes if needed. You can also choose a naming style (e.g., "snake" [default], "lower_camel", "upper_camel", "screaming_snake"). It works on data.frames/tibbles (and there’s `make_clean_names` for cleaning a character vector of names).

```{r reading in files}
library(janitor)

# Import raw file
asthma_mortality_raw <- read_csv("data/asthma_mortality.csv")  

# Output column names for raw dataset
names(asthma_mortality_raw)

# Import same file and pipe it through clean_names() [This is a best practice]
asthma_mortality <- read_csv("data/asthma_mortality.csv") %>% clean_names()

# Output column names cleaned dataset
names(asthma_mortality)
```


Reading in files is simple and just requires some knowledge of the location of your folder. Note that the dataset location above is in relation to the location of your .rmd file; the .rmd file is in the same folder that includes "data", so you don't need to specify any folder that came before. This is an example of a **relative path**, whereas specifying the entire detailed file location from your computer's hard drive would be an **absolute path**.  

A useful practice when you're familiarizing yourself with reading in files is actually going to the file in the RStudio "Files" tab, clicking on the dataset, and selecting "Import Dataset". It'll give you a window that has the **absolute path** at the top under File/URL and a code preview with the **relative path**. 

```{r preview files}
# Preview dataset
head(asthma_mortality)
```
...
What are the unique vales in the "years", "strata", and "age_group" variables? The first is done for you, and returns the unique values in the dataset _asthma_mortality_ and its variable _year_.

```{r explore data}

?unique

unique(asthma_mortality$years)

unique(asthma_mortality$strata)

unique(asthma_mortality$age_group)
...
data <- data %>%
  mutate(
    ...
    population = if_else(is.nan(population), NA_real_, population)
  )
...
data <- data %>%
  mutate(
    ...
    missing = if_else(is.na(population), "Yes", "No")
  )
```
